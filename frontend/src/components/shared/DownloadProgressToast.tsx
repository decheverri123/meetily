...
function DownloadToastContent({
  ...
  return (
    <div className="flex items-center gap-3 w-full max-w-sm bg-card/95 rounded-lg shadow-lg shadow-black/40 border border-border/10 backdrop-blur-2xl p-3 relative">

      {/* Icon */}
      <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${isComplete ? 'bg-success/15' : hasError ? 'bg-destructive/15' : 'bg-secondary/10'
        }`}>
        {isComplete ? (
          ...
          <ArrowBigDownDash className="size-5 text-primary " />
        )}
      </div>
      ...
      <div className="flex-1 min-w-0">
        ...
        {hasError ? (
          ...
          <>
            {/* Progress bar */}
            <div className="w-full h-1.5 bg-secondary/10 rounded-full overflow-hidden mb-1.5">
              <div
