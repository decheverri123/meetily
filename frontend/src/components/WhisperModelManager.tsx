...
export function ModelManager({
  ...
  if (error) {
    return (
      <div className={`bg-destructive/15 border border-destructive/20 rounded-lg p-4 ${className}`}>
...
function ModelCard({
  ...
  return (
    <motion.div
      ...
      className={`
        relative rounded-lg border-2 backdrop-blur-2xl transition-all cursor-pointer
        ${isSelected && isAvailable
          ? 'border-primary/40 bg-primary/15 text-foreground'
          : isAvailable
            ? 'border-border/10 hover:border-border/20 bg-secondary/5'
            : 'border-border/10 bg-secondary/[.03]'
        }
    ...
    >
      {/* Recommended Badge */}
      {isRecommended && (
        <div className="absolute -top-2 -right-2 bg-primary text-background text-xs px-2 py-0.5 rounded-full font-medium">
          Recommended
        </div>
      )}
      ...
      <div className="p-3">
        <div className="flex items-start justify-between mb-2">
          <div className="flex-1">
            ...
            <div className="flex items-center gap-2 flex-wrap mb-2">
              ...
              {isSelected && isAvailable && (
                <motion.span
                  initial={{ scale: 0 }}
                  animate={{ scale: 1 }}
                  className="bg-primary text-background px-2 py-0.5 rounded-full text-xs font-medium flex items-center gap-1"
                >
                  ✓
                </motion.span>
              )}
            ...
            {/* Model Specs */}
            <div className="flex items-center space-x-4 text-sm text-foreground/80 ml-9 mt-1.5">
          ...
          <div className="ml-4 flex items-center gap-2">
            ...
            {isMissing && (
              <button
                ...
                className="bg-primary text-background px-3 py-1.5 rounded-md text-sm font-medium hover:bg-primary/90 transition-colors"
              >
                Download
              </button>
            )}
            ...
            {downloadProgress === null && isError && (
              <button
                ...
                className="bg-destructive text-white px-3 py-1.5 rounded-md text-sm font-medium hover:bg-destructive/90 transition-colors"
              >
                Retry
              </button>
            )}
            ...
            {isCorrupted && (
              <div className="flex gap-2">
                <button
                  ...
                  className="bg-amber-500/20 text-amber-400 px-3 py-1.5 rounded-md text-sm font-medium hover:bg-amber-500/30 transition-colors"
                >
                  Delete
                </button>
                <button
