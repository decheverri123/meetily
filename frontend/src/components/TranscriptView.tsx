...
export const TranscriptView: React.FC<TranscriptViewProps> = ({ transcripts, isRecording = false, isPaused = false, isProcessing = false, isStopping = false, enableStreaming = false }) => {
  ...
  return (
    <div className="px-4 py-2">
      ...
      <AnimatePresence>
        {isRecording && (
          <div className="sticky top-4 z-10 bg-background/90 backdrop-blur-md pb-2">
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1">
                {isStreaming ? (
                  ...
                  <div className="glass-card border-l-2 border-l-primary px-3 py-2">
                  ...
                  <div className="relative">
                    <p className="text-base text-foreground/80 leading-relaxed" style={{ visibility: 'hidden' }}>
                      {sizerText}
                    </p>
                    <p className="text-base text-foreground/80 leading-relaxed absolute top-0 left-0">
                      {displayText}
                    </p>
                  </div>
                )}
      ...
      {!isStopping && isRecording && !isPaused && !isProcessing && transcripts.length > 0 && (
        ...
        >
          <div className="w-2 h-2 bg-destructive rounded-full animate-rec-pulse"></div>
          ...
          {isRecording ? (
            <>
              <div className="flex items-center justify-center mb-3">
                <div className={`w-3 h-3 rounded-full ${isPaused ? 'bg-muted-foreground' : 'bg-destructive animate-rec-pulse'}`}></div>
              </div>
