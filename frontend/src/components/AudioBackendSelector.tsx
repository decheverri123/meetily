  ...
  if (loading) {
    return (
      <div className="animate-pulse">
        <div className="h-4 bg-secondary/10 rounded w-32 mb-2"></div>
    ...
    <div className="space-y-2">
      <div className="flex items-center gap-2">
        ...
        <div className="relative">
          ...
          {showTooltip && (
            <div className="absolute z-10 left-6 top-0 w-64 p-3 text-xs border border-border/10 bg-background/95 backdrop-blur-2xl text-foreground rounded-lg shadow-lg shadow-black/20">
      ...
      {error && (
        <div className="p-2 text-xs text-destructive bg-destructive/15 border border-destructive/30 rounded-md">
          {error}
        </div>
      )}
          ...
          return (
            <label
              ...
              className={`flex items-start p-3 border rounded-lg transition-all ${
                currentBackend === backend.id
                  ...
                  : 'border-border/10 hover:border-border/25 bg-secondary/5'
            ...
            >
              <input
                ...
                disabled={isDisabled}
                className="mt-1 h-4 w-4 accent-primary focus:ring-primary border-border/20"
              />
              <div className="ml-3 flex-1">
                <div className="flex items-center justify-between">
                  ...
                  {isCoreAudio && (
                    <span className="text-xs font-medium text-muted-foreground bg-secondary/10 px-2 py-0.5 rounded">
                      Disabled
                    </span>
                  )}
                </div>
                <p className="mt-1 text-xs text-foreground/80">{backend.description}</p>
              </div>
            </label>
          );
        })}
