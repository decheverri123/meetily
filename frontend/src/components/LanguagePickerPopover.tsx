...
export function LanguagePickerPopover({
  ...
  return (
    <div
      ref={containerRef}
      className="w-72 glass-panel bg-background/95 shadow-lg shadow-black/20 overflow-hidden"
      role="dialog"
      aria-label="Pick summary language"
    >
      <div className="flex items-center gap-2 px-3 py-2.5 border-b border-border/10">
      ...
      <div className="max-h-80 overflow-y-auto py-1">
          ...
          <>
            ...
            {recentsResolved.map((opt) => (
              <button
                ...
                className={`flex w-full items-center justify-between px-3 py-1.5 text-sm hover:bg-secondary/20 text-left ${
                  value === opt.code ? "text-primary font-medium" : "text-foreground/80"
                }`}
              >
                <span>
                  {opt.label}{" "}
        ...
        {showAuto && (
          <button
            ...
            className={`flex w-full items-center justify-between px-3 py-1.5 text-sm hover:bg-secondary/20 text-left ${
              value === null ? "text-primary font-medium" : "text-foreground/80"
            }`}
          >
