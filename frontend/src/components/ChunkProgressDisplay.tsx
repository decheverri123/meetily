...
export function ChunkProgressDisplay({
  ...
  const getChunkStatusColor = (status: ChunkStatus['status']) => {
    switch (status) {
      ...
      case 'pending':
      default:
        return 'text-muted-foreground bg-secondary/5 border-border/10';
    }
  };
    ...
    <div className={`glass-card p-4 ${className}`}>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-3">
          ...
          {isPaused && (
            <span className="bg-amber-500/15 text-amber-400 px-2 py-1 rounded-full text-xs font-medium">
              Paused
            </span>
          )}
        </div>
        ...
        <div className="flex items-center space-x-2">
          {!isPaused ? (
            <button
              onClick={onPause}
              className="bg-amber-500/15 text-amber-400 hover:bg-amber-500/25 px-3 py-1 rounded text-sm transition-colors"
            ...
            <button
              onClick={onResume}
              className="bg-success/15 text-success hover:bg-success/25 px-3 py-1 rounded text-sm transition-colors"
            >
              Resume
            </button>
          )}
          ...
          <button
            onClick={onCancel}
            className="bg-destructive/15 text-destructive hover:bg-destructive/25 px-3 py-1 rounded text-sm transition-colors"
          >
            Cancel
          </button>
        </div>
...
export function ChunkProgressMini({ progress, className = '' }: { progress: ProcessingProgress; className?: string }) {
  ...
  return (
    <div className={`glass-card p-3 ${className}`}>
      ...
      <div className="w-full bg-secondary/10 rounded-full h-1.5 mb-2">
        <div
